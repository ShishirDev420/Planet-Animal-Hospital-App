...
export function createWalletHandler(getServices=database) {return async(req:any,res:any)=>{
 ...
 try {
  ...
  if(type==='registerInvoice') {
    ...
    const invoiceId=hash(b.reference),ref=db.doc(`careInvoices/${invoiceId}`),invoice:Invoice={ownerUid:owner,reference:b.reference,eligiblePaise:b.eligiblePaise,status:'open',createdBy:actor.uid};
    await db.runTransaction(async tx=>{const old=await tx.get(ref);if(old.exists){if(old.data()?.ownerUid!==owner||old.data()?.eligiblePaise!==b.eligiblePaise)throw new CareError(409,'Invoice already recorded with different details.');return;}tx.create(ref,invoice);tx.create(root.collection('entries').doc(),{type,invoiceId,actorUid:actor.uid,at:now});});return send(200,{invoiceId});
  }
  const result=await db.runTransaction(async tx=>{
    ...
    if(type==='reserve') {
      ...
      const [r,i]=await Promise.all([tx.get(rRef),tx.get(iRef)]);
      if(r.exists){if(r.data()?.invoiceId!==invoiceId||r.data()?.points!==b.points)throw new CareError(409,'Retry does not match the original reservation.');return {wallet,reservation:r.data()};}
      ...
      const next=reserveDiscount(wallet,i.data() as Invoice,b.points,reservationId,invoiceId,actor,now);
      tx.set(root,next.wallet);tx.create(rRef,next.reservation);tx.update(iRef,{reservationId});tx.create(root.collection('entries').doc(`reserve-${reservationId}`),{type,points:b.points,invoiceId,actorUid:actor.uid,at:now,policyVersion:WALLET_POLICY.version});return next;
    }
    if(['apply','release','reverse'].includes(type)) {
      ...
      tx.create(root.collection('entries').doc(`${type}-${reservationId}`),{type,points:current.points,paise:current.paise,invoiceId:current.invoiceId,billingReference:b.billingReference||null,actorUid:actor.uid,at:now,policyVersion:WALLET_POLICY.version});return next;
    }
