...
export async function assistantRequest(body?:Record<string,unknown>) {
  ...
  const response=await fetch('/api/assistant',{method:body?'POST':'GET',headers:{Authorization:`Bearer ${await user.getIdToken()}`,'Content-Type':'application/json'},...(body?{body:JSON.stringify(body)}:{})});
  if(!response.headers.get('content-type')?.includes('application/json'))throw new Error('AI assistance is unavailable. Recorded care remains available.');
  const data=await response.json();if(auth.currentUser?.uid!==user.uid)throw new Error('Account changed. Reopen the assistant.');
  if(!response.ok)throw new Error(data.error||'Please retry the same question.');return data;
}
...
function AssistantSession({agent,requestService}:{agent:string;requestService:typeof assistantRequest}) {
  const care=useCare();const [prompt,setPrompt]=useState(''),[consent,setConsent]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(''),[answer,setAnswer]=useState<AssistantDraft|null>(null),[remaining,setRemaining]=useState<any>(null);
  const request=useRef({prompt:'',id:''}),mounted=useRef(true);
  useEffect(()=>{mounted.current=true;void requestService().then(d=>{if(mounted.current)setRemaining(d.allowance);}).catch(()=>{});return()=>{mounted.current=false;};},[]);
  ...
  return <section aria-label={`${agent} AI discussion`} className="liquid-glass relative rounded-[2rem] border border-white/10 p-5 text-white min-w-0">
    ...
    <p className="mt-3 text-xs text-planet-yellow" role="status">{remaining?`${remaining.dailyRemaining} of ${remaining.dailyLimit} free requests left today · ${remaining.monthlyRemaining} of ${remaining.monthlyLimit} this month. Resets at UTC boundaries.`:'Free allowance: up to 5 requests a day, 20 a month. Sign in to check availability.'}</p>
