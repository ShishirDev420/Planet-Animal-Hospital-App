...
export default function PlanComparison({ onExplore }: { onExplore: (id: PlanId) => void }) {
  ...
  return <section id="compare-plans" className="mt-12 scroll-mt-6" aria-labelledby="comparison-title">
    ...
    <p className="text-sm text-white/75 leading-relaxed max-w-2xl mt-3">Start with the benefits that matter to you. Compare all three plans, or focus on what changes between them.</p>
    ...
    <p className="text-xs text-white/65 leading-relaxed mt-5">Items marked “Confirm” need clarification before joining. The existing Premium listings differ on deworming, blood panels and included grooming; Calendar sync availability also needs confirmation. Your veterinarian advises which care is appropriate for your pet.</p>
  </section>;
}
