...
export function HomeRewardCard({ balance, notice, onWallet }: { balance: Balance | null; notice: string; onWallet: () => void }) {
  ...
  return <section aria-label="Paw Points progress" className="home-reward-card">
    <div className="home-reward-copy">
      <span className="home-reward-kicker">CARE WALLET</span>
      <h2>Paw Points</h2>
      <p>Clinic-verified care, reflected in your wallet.</p>
      <div className="home-reward-points"><span>{points === null ? '—' : points.toLocaleString('en-IN')}</span><div><strong>{points === null ? 'Balance unavailable' : 'Available points'}</strong><small>{points === null ? 'Check your wallet for details' : 'Ready when the clinic approves a reward'}</small></div></div>
      <div className="home-reward-next"><span>{ready ? next ? 'Next approved reward' : 'Current approved reward' : 'Reward status'}</span><strong>{ready ? `${next?.percent ?? current?.percent}% off the entire bill` : notice || 'Appears when approved by the clinic'}</strong>{ready && next && <small>{(next.points - points!).toLocaleString('en-IN')} points to go</small>}</div>
      ...
      <button type="button" className="home-reward-link" onClick={onWallet}>View care wallet <ArrowRight size={16}/></button>
      <p className="home-reward-fineprint">Points are earned only after eligible care is completed and verified by the clinic.</p>
      {balance?.paused && <p className="home-reward-warning">The clinic needs to review your wallet before points can be used.</p>}
    </div>
    <div className="home-reward-visual" aria-hidden="true"><div className="home-reward-orbit"><span className="home-reward-orbit-ring ring-one"/><span className="home-reward-orbit-ring ring-two"/><span className="home-reward-orbit-ring ring-three"/><span className="home-reward-core"><PawPrint size={38} strokeWidth={1.6}/></span><span className="home-reward-star star-one">✦</span><span className="home-reward-star star-two">✧</span></div><span className="home-reward-level"><PawPrint size={12}/> {ready ? `LEVEL ${level} OF ${tiers.length}` : 'PAW POINTS'}</span></div>
  </section>;
}

export default function HomeRewardProgress({ demo, onWallet }: { demo: boolean; onWallet: () => void }) {
  ...
  return <HomeRewardCard balance={balance} notice={notice} onWallet={onWallet}/>;
}
