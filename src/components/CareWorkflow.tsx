  ...
  return <section aria-label="Recorded care follow-through" className="relative z-10 my-5 rounded-[2rem] border border-white/10 bg-black/40 p-5 text-white sm:p-6">
    ...
    {loading ? <p role="status" className="mt-4">Loading recorded care…</p> : error ? <div className="mt-4"><p role="alert">{error}</p><button className={`${careButton} mt-3`} onClick={() => void refresh()}>Retry care service</button></div> : state && <>
      ...
      {next ? <>
        ...
        <button className={`${careButton} mt-2`} disabled={busy || !date} onClick={()=>void run({type:'book', milestoneId:next.id,requestedFor:new Date(date).getTime()},'Request saved. The clinic must confirm availability; no points are awarded for booking.')}>{next.booking.status === 'confirmed' ? 'Request rescheduling' : 'Request appointment'}</button>
      ...
      <div className="mt-5 border-t border-white/10 pt-4"><p className="text-sm font-bold">Pawl · Verified pilot rewards</p><p className="mt-2 text-sm">{state.ledger.reduce((n,l)=>n+l.points,0)} earned points · ₹{(state.ledger.reduce((n,l)=>n+l.creditPaise,0)/100).toFixed(2)} recorded clinic credit</p><p className="mt-2 text-xs text-white/65">Existing wallet amounts remain separate until reconciled. Credit redemption needs the clinic's billing integration; it is unavailable in this pilot. Checkup entitlements are pet-specific and are not inferred from points.</p><p className="mt-2 text-sm">{state.entitlements.filter(e=>e.petId===petId).length} recorded checkup entitlements for this pet.</p></div>
    </>}
