      ...
      <div className="px-6 pt-12 pb-8 relative">
        ...
        >
          ...
          <h1 className="text-3xl font-black tracking-tight leading-tight mb-4">
            Stop emergencies <br/>
            <span className="text-slate-400">before they start.</span>
          </h1>
          ...
          <div className="glass p-4 rounded-2xl border-l-4 border-l-planet-yellow">
            <p className="text-sm font-medium text-slate-800">
              <Zap className="inline text-planet-yellow mr-1" size={16}/>
              All plans instantly unlock the <strong className="text-black">2x Points Multiplier</strong> for every visit.
            </p>
          </div>
        </motion.div>
      </div>
...
function PlanCard({ title, price, period, description, features, isPopular, delay }: any) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay }}
      className={`relative rounded-3xl p-6 ${isPopular ? 'bg-slate-900 text-white shadow-2xl shadow-slate-900/30' : 'glass-card'}`}
    >
      {isPopular && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-planet-yellow text-black text-xs font-black px-4 py-1 rounded-full shadow-lg">
          MOST POPULAR
        </div>
      )}
      <h3 className={`text-xl font-bold mb-1 ${isPopular ? 'text-white' : 'text-slate-800'}`}>{title}</h3>
      <p className={`text-sm mb-4 ${isPopular ? 'text-slate-400' : 'text-slate-500'}`}>{description}</p>
